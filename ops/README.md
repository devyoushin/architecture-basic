...
| 폴더 | 내용 |
|------|------|
| `runbooks/` | Nginx, DB, CDN origin, scale-out 의사결정 등 장애/부하 대응 절차 |
...
4. `runbooks/cdn-origin-overload-response.md`로 CDN cache miss와 origin 부하 분리
5. `nginx/nginx-tuning.conf`와 `nginx/sysctl-nginx.conf`로 설정 후보 검토
6. `database/mysql-high-load-queries.sql`, `database/postgres-high-load-queries.sql`로 DB 내부 상태 확인
7. `load-test/` 스크립트로 변경 전/후 성능 비교
